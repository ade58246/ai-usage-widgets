...
def _draw_icon(size: int) -> QImage:
    ...
    painter.setBrush(QColor("#2B2421"))
    ...
    pen = QPen(
        QColor("#79675F"), max(2.0, size * 0.075), Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap
    )
    painter.setPen(pen)
    painter.setBrush(Qt.BrushStyle.NoBrush)
    painter.drawArc(gauge_rect, 210 * 16, -240 * 16)

    pen.setColor(QColor("#E58B6B"))
    painter.setPen(pen)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate the Claude Usage Widget icon")
